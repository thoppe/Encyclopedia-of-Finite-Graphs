...
\newcommand{\OEISedits}{XXX}
\newcommand{\OEISprimary}{XXX}
...
Sloane, as a graduate student, began collecting integer sequences while studying combinatorial problems\cite{sloane2003line}. 
...
Some invariants, like the independence number \OEIS{A243781}, have produced sequences that were hitherto unknown to OEIS but amenable under the scope of this project.
...
A full table of all sequences submitted to the OEIS can be found in Appendix \ref{app:submittedseq} and relations in Appendix \ref{app:relations}.
...
\item A \textit{vertex-induced subgraph} is a subset of vertices and edges who are common to all members of this subset. 
...
Additionally, we have checked for the induced subgraphs $K_5$, the cycle graphs $C_4, C_5, C_6$ and the following named graphs: the \bowtiegraph, \diamondgraph, \bannergraph, and the open-bowtie graph. 
\note{Added picture of these graphs}
\note{Mention k max clique here}
...
\item \textit{Independent edge/vertex sets}, \textit{independence number}, \textit{Hosoya index}.
\note{Complete def. here}
