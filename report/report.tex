...
\newcommand{\OEISeditsExtended}{six\xspace}
...
\newcommand{\VARisuniquespectrum}
{\indicatorfunctionX{\VARuniquespectrum}}
...
\thanks{\noindent
  Tel.: +1-301-496-2618; Fax: +1-301-496-0825.\\
  Electronic address: \texttt{hoppeta@mail.nih.gov}.
}}
\author[2]{Anna Petrone}
\affil[1]{National Institutes of Health, National Institute of Diabetes and Digestive and Kidney Diseases, 9000 Rockville Pike, Bethesda, MD, 20892}
...
\OEISeditsMain new sequences were added and \OEISeditsExtended sequences were extended.
...
To this end, we have created the \textit{Encyclopedia of Finite Graphs} (henceforth Encyclopedia) which includes a database of graphs and invariants\cite{Travis2014SimpleConnectedGraphs}.
The software to fully populate the database has been released under an open source license\cite{Travis2014Encyclopdia}.
The intention is for new invariants to be added to the project as various algorithms become available.
...
Two queries of the same class are subsets of each other $\QUERY(f,\CLASS) \subseteq \QUERY(g, \CLASS)$, if $f(\CLASS_i) \subseteq g(\CLASS_i)$ for all $i\ge0$. 
Equality of two queries $\QUERY(f,\CLASS) = \QUERY(g,\CLASS)$, implies $\QUERY(f,\CLASS) \subseteq \QUERY(g,\CLASS)$ and $\QUERY(g,\CLASS) \subseteq \QUERY(f,\CLASS)$. 
We say that a relation between two invariant conditions is \textit{suggestive} to order $n$, $\QUERY(f,\CLASS) \subseteq_n \QUERY(g,\CLASS)$, if $f(\CLASS_i) \subseteq g(\CLASS_i)$ for $0 \le i \le n$.
They are \textit{exclusive} to order $n$, $\QUERY(f,\CLASS) \cap_n \QUERY(g,\CLASS)$, if $f(\CLASS_i) \cap g(\CLASS_i) = \emptyset$ for $0 \le i \le n$.
We can quickly filter candidate relations by noting that it is necessary but not sufficient for the same conditions to hold for sequences, \eg $\SEQ_a \ne \SEQ_b \implies  \QUERY(f,\CLASS) \ne \QUERY(g,\CLASS)$.
...
Using the \texttt{geng -c} command from \texttt{nauty}\cite{mckay2014practical}, we enumerated the simple connected graphs up to order $n \le 10$.
...
The invariants that were computable with integer or linear programming were done with \texttt{PuLP}\cite{mitchell2011pulp}.
...
Most of the invariants were stored as integers; the invariant database is about $850$ MB when compressed.
...
Since each invariant calculation was an independent problem, the computation was especially well-suited for parallelization across multiple processing cores.
...
We placed an index on each of the invariant columns, at the expense of disk space, to allow for fast querying.
...
We refer to sequences with only a single invariant condition as primary sequences, while sequences involving two or more invariant conditions we call secondary sequences.
...
For each pair of invariant conditions, the sets of graphs related to the sequences were compared for set equality, exclusivity, or as subsets.
...
From a technological standpoint, it is currently possible to extend the database to simple connected graphs of larger orders.
...
We plan to create a web interface that would allow users to query a specific graph based on a commonly assumed name (e.g. the ``Petersen graph") or query the set of graphs that match a selection of invariant conditions.
...
Unless otherwise noted, all invariants listed below in italics were computed and stored in the Encyclopedia.
Many of them were already present in the OEIS and their corresponding integer sequences were cross-referenced as a consistency check.
...
A graph is \textit{integral} if all the values of the adjacency spectrum are integral, $\lambda_k \in \mathbb{Z}$ for all $k$. 
A graph is \textit{unique} if each eigenvalue in the spectrum is distinct.
...
The total number of independent edge sets is sometimes referred to as the number of \textit{edge matchings}, or the \textit{Hosoya index}\cite{hosoya1971topological}.
For graphs that are bounded in treewidth the Hosoya index is fixed-parameter tractable.
In general however, the Hosoya index is $\mathcal{\#P}$-complete\cite{jerrum1987two}.
...
A \textit{proper $k$-coloring} of a graph is an assignment of $k$ colors to each vertex so that no two adjacent vertices have the same color. 
...
The smallest $a$ such that the graph can be assigned to $b$ colors is the \textit{$b$-fold chromatic number} and is denoted by $\VARfractionalchromaticnumber{b}$. 
...
The integral chromatic number need not be equal to its fractional counterpart, in fact 
...
A \textit{vertex subgraph} or simply a subgraph, is a subset of vertices and edges which are common to all members of this subset. 
Let $g, h \in \CLASS$ be graphs from the same class and denote $s(g, h)$ as the number of vertex sets that form subgraphs of $h$ in $g$.
...
Illustrated below from left-to-right are
...
The definition for each invariant is given in Section \ref{app:invariants}.
...
$\VARtree,\VARhamiltonian,\VAReulerian, \VARintegral, \VARuniquespectrum$ & 
properties: tree, Hamiltonian, Eulerian, integral, unique \\
...
$C_n, K_n$ & 
Named graphs (of order $k$): cycle graph, complete graph \\
$B_5, \subgraphDIAMOND, \subgraphBOWTIE, \subgraphOPENBOWTIE$ &
Named graphs: Bull, diamond, bowtie, open-bowtie \\
\end{longtable}
...
\OEIS{A242952} & $\VARisuniquespectrum =1$ & 1 & 1 & 1 & 3 & 11 & 54 & 539 & 7319 & 209471 & 10000304 \\
\OEIS{A242953} & $\VARisuniquespectrum =0$ & 0 & 0 & 1 & 3 & 10 & 58 & 314 & 3798 & 51609 & 1716267 \\
