...
The second-largest eigenvalue of is called the \textit{spectral gap}.
...
A graph is \textit{distance regular} if, for all pairs of vertices $v$, $w$ and any pair of integers $i,j = 0,1,\ldots,d$ where $d$ is the graph diameter, the number of vertices from $v$ at a distance $i$ and the number of vertices from $w$ at a distance $j$ depend only on $i, j$ and the distance between $v$ and $w$, independent of the choice of $v$ and $w$\cite{brouwer2012distance}.
...
The distinct sequences were those formed by considering the number of unique values possible at a fixed order.
...
\begin{longtable}{@{\extracolsep{\fill}} l l r r r r r r r r r r}
\toprule
OEIS & Invariant & 
\multicolumn{10}{l}{Sequence: \ $S_1, S_2, \ldots, S_{10}$, terms in $(\cdot)$ are new.} 
\\
\midrule\bottomrule
...
\begin{longtable}{@{\extracolsep{\fill}} l l l r r r r r r r r r r}
\toprule
OEIS & Invariant & & 
