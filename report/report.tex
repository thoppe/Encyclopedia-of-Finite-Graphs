...
\item fill in gaps of invariants (eg aut group)
...
\item The largest database, holding all graphs with 10 nodes (there are XXX such graphs) is XXGB. (depends on if you use the vertical or indexed representation. probably better to quote the size without indexes, because that has more to do with the number of graphs and not design decisions. ) 
\end{itemize}
...
\item invariants are stored as integers: either the value, eg girth, or as a 0 or 1, eg isbipartite. Special invariants: tutte polynomial 
\end{itemize}
