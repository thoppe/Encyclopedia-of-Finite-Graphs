...
This is similar the GraPHedron project\cite{melot2008facet}, which attempts to formulate conjectures by searching for graphs bounded by an inequality or constraint.
...
% isn't \SEQ the operator? In other words I don't understand why we need \SEQ_a and \SEQ_b. Wouldn't it just be \SEQ(f, C), \SEQ(g, C)?
...
In this second database, each graph is stored as a row and each column is an indexed invariant. % I don't think the phrase "indexed invariant" makes sense. 
% I would say "... and each column contains the value of a particular invariant for each graph. We place an index on each of the invariant columns to allow for fast querying, at the expense of disk space (which is readily available, and cheap).
