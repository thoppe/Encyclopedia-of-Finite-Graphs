...
We have computed the Encyclopedia of Finite Graphs, a exhaustive enumeration of all simple connected graphs and their invariant values for graphs of order ten or less.
...
Larger orders would require prohibitive storage requirements as the number of graphs grows quickly (\OEIS{A001349}).
...
An extension of the Encyclopedia could incorporate these graphs that others have already determined and solve for their respective invariants.
...
Large collections of graph invariants have been compiled (though not computed) under the Global Constraint Catalog\cite{beldiceanu2005global} which also extensively details the relations among invariants. 
...
For example \note{finish this part here.}

As the Encyclopedia grows however, the published static database will be insufficient to handle these extensions.
We plan to create a web interface that would allow users to query graphs based on invariants or their names (e.g. ``Peterson graph").
