...
These databases range from those of mathematical and algorithmic interest \cite{de2003large, brinkmann2013house}, to those cataloging structures found in the applied sciences such as ChemSpider\cite{pence2010chemspider} or RNA topologies\cite{pence2010chemspider, gan2004rag}.
Due to the rapid growth in the number of unique isomorphic graphs however, the currently available databases are specialized in the class of graphs considered; 
a judicious choice often restricts the study to an interesting and and more manageable subset.

We proceed with the assumption however, that \textit{a priori} all graphs could be interesting given the right question.
This is underlies the GraPHedron project\cite{melot2008facet}, an attempt to formulate and bound conjectures on prior knowledge.
...
For example, the integer sequences formed by the invariants can be systematically explored and compared to those already known in the Online Encyclopedia of Integer Sequences (OEIS)\cite{sloane2003line}.
...
The framework of GraPHedron can be used to amplify the predictions.

The creation of a large, centralized database will serve as a useful reference for benchmarking various algorithms.
Additionally a comprehensive database provides pedagogic value, representative graphs from any considered sequence can be rapidly visualized.
To this end, we have created the Encyclopedia of Finite Graphs, a database of invariants and software needed to fully reproduce each invariant\cite{Travis2014Encyclopdia}. 
...
An immediate tangible one can collect from a graph invariant database are the integer sequences formed by matching the number of graphs to the invariant for a given order.
Many such sequences formed from graph invariants have been found and cataloged already in another database, the Online Encyclopedia of Integer Sequences (OEIS)\cite{sloane2003line}.
Created by Neil Sloane in 1964 as a graduate student, Sloane began collecting integer sequences while studying combinatorial problems.
...
We have computed simple invariants that are computable in polynomial time, to those that are known to be $\mathcal{\#P}$-complete.
While a few sequences were merely extended, invariants such as the the independence number were were hitherto unknown to OEIS and have produced novel sequences (\OEIS{A243781}-\OEIS{A243784}).
...
Unless otherwise stated, any referenced graph from this point is assumed to be simple and connected.
...
The largest class computed of order 10, requires 45 bits. 
This can be efficiently stored in a 64 bit unsigned integer using two's complement representation.
...
We found however, that it was useful to store certain non integer invariants, such as the Tutte polynomial, as a separate text field.
...
Hence, the build the sequences themselves a second database is created where each graph is stored as a row and each column is an indexed invariant.
...
A sequence that with only a single invariant condition is called a primary sequences, while a sequencing involving two or more invariants are called secondary sequences.
...
For example, one of the equality relations tell us a graph is a tree $\Leftrightarrow$ a graph has zero girth.
This obviously follows from the fact that trees contain no cycles and the girth of a graph is the length of the shortest cycle. 
...
%Many sequences were already present in the OEIS served as a useful check on the accuracy of our programs.
