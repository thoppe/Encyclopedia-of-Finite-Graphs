...
While we explore a basic set of relations for the invariants, the framework of GraPHedron can be used to amplify the predictions.
...
While some sequences were merely extended, many invariants like the independence number, have produced sequences that were hitherto unknown to OEIS \OEIS{A243781}-\OEIS{A243784}.
...
Since the graphs in $\SIMPLECLASS$ are loopless and undirected, the edge incidence information for each graph requires $n(n+1)/2$ bits for storage.
The largest class computed, $\SIMPLECLASS$ requires 45 bits and can be efficiently stored in a 64 bit unsigned integer with two's complement representation, commonly supported by many database programs. 
...
Most of the invariants, especially those used in integer sequence calculations, are stored as integers.
We found however, that it was useful to store certain polynomials such as the Tutte polynomial as a separate text field.
\note{mention the size of the database?}

The database construction first proceeds by the enumeration of graphs themselves.
Once this has been done, the database checks against a reference table for invariants that have not been computed and allocates the processing to multiple computers.
...
Once the computation is completed for any finite set of invariants, a new database is created where each graph is stored as a row and each column is an indexed invariant.

Sequences are constructed by first enumerating all the unique values for each invariant and applying a condition.
For example, sequence \OEIS{241454} describes the graphs whose automorphism group is equal to 2 while sequence \OEIS{A086216} describes graphs with a vertex connectivity greater or equal to three. 
The sequence that involve only a single invariant are called primary sequences, while those involving two or more invariants are called secondary.
For a sequence to be considered for submission to the OEIS it must have at least four non-zero entries. 

The suggestive and exclusive relations described in the previous section are built up in a similar way.
For each pair of invariants conditions describing two sequences, the set of graphs related to the sequences are compared.
These relations provided a useful check on the accuracy of the algorithms.
For example, one of the equality relations tell us that all trees have zero girth and all graphs of zero girth are trees which obviously follows from the fact that trees contain no cycles and the girth of a graph is the length of the shortest cycle. 
