...
Similar to the \textit{House of Graphs} project \note{REF}, we say that a relation between two invariants is \textit{suggestive to order $n$}, $\SEQ_a \subseteq_n \SEQ_b$, if $f(\CLASS_i) \subseteq g(\CLASS_i)$ for $0 \le i \le n$.
...
Using the \texttt{geng -c} command from \texttt{nauty} \note{REF}, we enumerated the class of $\SIMPLECLASS$ up to order $n \le 10$.
