...
% is it ok to talk about the OEIS before we say what it is? 
...
% Makybe you can say "henceforth Encyclopedia" (no italics) and then only refer to it as Encyclopedia in the rest of the text
...
% so later in the table, we can get rid of \Theta and just say that T = f_tree, H = f_hamiltonian ...
%
\begin{align}
...
% This is confusing because sequences has two meanings, a sequence of sets of graphs, \SEQ, and an integer sequence, S. maybe we can call \SEQ a query, since everywhere else in teh paper, sequence refers to an integer sequence. 
...
% isn't \SEQ the operator? In other words I don't understand why we need \SEQ_a and \SEQ_b. I think it should be "let \SEQ_a = \SEQ(f, C), and \SEQ_b = \SEQ(g, C)"
...
However, we found it useful to store certain non-integer invariants, such as the Tutte polynomial in a separate, specialized database. %not sure about the hyphenation rules but we have "non-zero" later, so I put a hyphen in non-integer
\note{mention the size of the database?}

%The database construction first proceeds with the enumeration of graphs themselves.
...
Then all of the invariants found in the invariant reference table were allocated to multiplue computers to be calculated.
...
% I don't think the phrase "indexed invariant" makes sense. 
% I would say "... and each column contained the value of a particular invariant for each graph. We placed an index on each of the invariant columns to allow for fast querying, at the expense of disk space (which was readily available, and cheap).
...
The invariant descriptions are broadly organized into sections of algebraic graph theory, topological graph theory, and invariants related to structure-like connectivity.  %again with the hyphens.. I'm not totally sure
